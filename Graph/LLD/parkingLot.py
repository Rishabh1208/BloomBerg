...
class ParkingLot:
    ...
    def processExit(self, ticket):
        ...
        print('Exit Completed For  : ', ticket.veh.num,
              ' Pay : ', int(ticket.payment.amount))
...
class Payment:
    ...
    def calAmount(self, inTime, outTime):
        ...
        amount += (outTime - inTime - 60) * \
            self.getRate()[1] if outTime - inTime - 60 > 0 else 0
        amount += (outTime - inTime - 120) * \
            self.getRate()[2] if outTime - inTime - 120 > 0 else 0
        return amount
...
class DisplayBoard:
    def show(self, P):
        for eachlevel in P.level:
            ...
            print('Car  : ', len(
                eachlevel.spots[VehicleType.CAR][SpotType.FREE]))
            print('Bike : ', len(
                eachlevel.spots[VehicleType.BIKE][SpotType.FREE]))
